...
TrashCan::TrashCan(std::string name, Vector2f initialCenterPosition) : StandardObject(name) {
	m_Tag = "trashcan";

	/************************
	***Transform Component***
	************************/
	m_TC = std::make_shared<TransformComponent>(initialCenterPosition, Vector2f(0.5f, 0.5f));
	m_HasTC = true;
