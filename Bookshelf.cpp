...
Bookshelf::Bookshelf(std::string name, Vector2f initialCenterPosition) : StandardObject(name) {
	m_Tag = "bookshelf";

	/************************
	***Transform Component***
	************************/
	m_TC = std::make_shared<TransformComponent>(initialCenterPosition, Vector2f(2.0f, 1.0f));
	m_HasTC = true;
